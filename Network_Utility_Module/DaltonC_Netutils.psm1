...
# Support functions: Function Get-ValidHosts calculates the number of valid hosts available for a given subnet
#===================================================
...
function Test-IPNetwork () {
    ...
    if ($SubnetMask.length -lt 3) {
        # <DEBUGGING> $SubnetMask.length
        # <DEBUGGING> $SubnetMask.gettype()
        [Net.IPAddress]$SubnetMask = Convert-CIDR $SubnetMask
        # <DEBUGGING> Write-Host -foreground "green" "the converted cidr mask is: " $SubnetMask
    }
    else {[Net.IPAddress]$SubnetMask = $SubnetMask}

    if (($IP1.address -band $SubnetMask.address) -eq ($IP2.address -band $SubnetMask.address)) {
        Write-Host -ForegroundColor "green" "The addresses you entered are on the same network."
        ...
        Write-Host -Foreground Red "The addresses you entered are not on the same network."
        "IP1 = " + $IP1.gettype()
        "IP2 = " + $IP2.gettype()
        "SubnetMask = " + $SubnetMask.gettype()
    }
...
function Get-ValidHosts {
    ...
    process {
        foreach ($c in $cidrMask) {
            $hostBits = 32 - $c
            $numValidHosts = ([math]::pow( 2, $hostBits)) - 2

            Write-Host -ForegroundColor "green" "The number of valid hosts on the /$c subnet is: " $numValidHosts
        }
    }
}
